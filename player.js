class Player {
  ...
  _seek(e) {
    if (!this._player.paused) {
      ...
      this._player.currentTime =
        (xPositionClicked * this._player.duration) / seekbar.offsetWidth;
    }
  }
  ...
  _previous() {
    ...
    const currentIndex =
      this._currentSrcIndex - 1 < 0 ? lastIndex : this._currentSrcIndex - 1;
  ...
  _next() {
    const lastIndex = this._sources.length - 1;
    const currentIndex =
      this._currentSrcIndex + 1 > lastIndex ? 0 : this._currentSrcIndex + 1;
    this._changeSrc(currentIndex);
    this._play();
  }
  ...
  _syncPlayingIcon() {
    const pastPlaying = document.querySelector(".playing");
    pastPlaying.classList.remove("playing");
    pastPlaying.firstElementChild.innerText = "play_arrow";

    const currentPlaying = document.querySelector(
      `[data-index="${this._currentSrcIndex}"`
    );
  ...
  _appendPlayList() {
    ...
    this._sources.forEach((source, i) => {
      ...
      song
        .querySelector('[data-action="changesource"]')
